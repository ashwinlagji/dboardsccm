...
function getKeywords(responseObj) {
    dbObject.collection('wordcount').find().sort({count:-1}).limit(500).toArray(function (err, docs) {
        var dataset=[];
        for(index in docs){
            var obj={};
            obj.text=docs[index].word;
            obj.size=docs[index].count;
            dataset.push(obj);
        }
